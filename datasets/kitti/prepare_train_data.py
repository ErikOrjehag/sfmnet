...
def dump_example(args, scene):
    ...
    for scene_data in scene_list:
        ...
        for sample in data_loader.get_scene_imgs(scene_data):
            ...
            imageio.imsave(dump_img_file, skimage.img_as_ubyte(img))
            if "pose" in sample.keys():
                poses.append(sample["pose"].tolist())
            if "depth" in sample.keys():
                dump_sparse_file = dump_dir/'{}.npy'.format(frame_nb)
                dump_dense_file = dump_dir/'{}_dense.npy'.format(frame_nb)
                np.save(dump_sparse_file, sample["depth"][0])
                np.save(dump_dense_file, sample["depth"][1])
