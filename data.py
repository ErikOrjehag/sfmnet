...
def _get_batch_loader_split(Class, path, batch, workers):
    random.seed(1337)
    dataset = Class(path)
    return data_loaders(dataset, 0.899, 0.001, 0.1, batch, workers)
...
def get_loader(args):
    if args.dataset == "kitti":
        return _get_loader(Kitti, kitti_path, args.batch, args.workers)
    elif args.dataset == "lyft":
        return _get_loader(Lyft, lyft_path, args.batch, args.workers)
    else:
        print("No dataset named: %s" % args.dataset)
        exit()